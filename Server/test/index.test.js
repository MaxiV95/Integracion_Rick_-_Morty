...
describe("test de RUTAS", () => {
  ...
  describe("GET /rickandmorty/login", () => {
      ...
      it("Responde con un objeto con la propiedad access en true si la información del usuario es válida", async () => {
          const response = await request.get('/rickandmorty/login?email=maximilianovanmegroot@gmail.com&password=asdasd1');
          expect(response.body).toEqual(access);
      });

      it("Responde con un objeto con la propiedad access en false si la información del usuario no es válida", async () => {
          const response = await request.get('/rickandmorty/login?email=maximilianovanmegroot@mail.com&password=asd321');
          access.access = false;
          expect(response.body).toEqual(access);
      });
  });
